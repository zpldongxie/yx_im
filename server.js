...
// js,css资源
app.use('/h5/dist', express.static(path.join(projPath, 'dist')))
// 图片资源
app.use('/h5/img', express.static(path.join(projPath, 'img')))

// 设置html作为渲染器
app.engine('html', ejs.__express);
app.set('view engine', 'html')

app.get('/h5/login.html', function (req, res, next) {
  res.render(path.join(projPath, 'login'))
})

app.get('/h5/regist.html', function (req, res, next) {
  res.render(path.join(projPath, 'regist'))
})

// 单页应用页面
app.get('/h5/index.html', function (req, res, next) {
  res.render(path.join(projPath, 'index'))
})

app.get('/', function (req, res, next) {
  res.redirect('/h5/index.html')
})

app.post('/h5/getlogger', (req, res) => {
